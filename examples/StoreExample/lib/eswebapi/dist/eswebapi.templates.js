angular.module('es.Web.UI').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('src/partials/esParamAdvancedString.html',
    "<label class=\"control-label es-param-label\" uib-tooltip={{::esParamDef.toolTip}} tooltip-placement=top tooltip-trigger=mouseenter>{{::esParamDef.caption}}</label><div class=row><div class=col-xs-12 ng-class=\"{'': esParamVal[esParamDef.id].paramValue.oper == 'NULL' || esParamVal[esParamDef.id].paramValue.oper == 'NOTNULL', 'col-md-4': esParamVal[esParamDef.id].paramValue.oper == 'RANGE', 'col-md-6': esParamVal[esParamDef.id].paramValue.oper != 'NULL' && esParamVal[esParamDef.id].paramValue.oper != 'NOTNULL' &&  esParamVal[esParamDef.id].paramValue.oper != 'RANGE'}\"><select class=\"form-control es-param-control\" kendo-drop-down-list ng-cloak k-ng-delay=esParamDef k-data-text-field=\"'caption'\" k-data-value-field=\"'value'\" k-auto-bind=true k-data-source=esGlobals.getesComplexParamFunctionOptions() k-value-primitive=true k-ng-model=esParamVal[esParamDef.id].paramValue.oper></select></div><div class=col-xs-12 ng-if=\"esParamVal[esParamDef.id].paramValue.oper != 'NOTNULL' && esParamVal[esParamDef.id].paramValue.oper != 'NULL'\" ng-class=\"{'col-md-6': esParamVal[esParamDef.id].paramValue.oper != 'RANGE', 'col-md-4': esParamVal[esParamDef.id].paramValue.oper == 'RANGE'}\"><input class=\"form-control es-param-control\" kendo-masked-text-box ng-required=\"esParamVal[esParamDef.id].paramValue.oper != 'RANGE' && esParamDef.required\" name={{::esParamDef.id}} ng-model=\"esParamVal[esParamDef.id].paramValue.value\"></div><div class=\"col-xs-12 col-md-4\" ng-if=\"esParamVal[esParamDef.id].paramValue.oper == 'RANGE'\"><input class=\"form-control es-param-control\" kendo-masked-text-box ng-model=\"esParamVal[esParamDef.id].paramValue.valueTo\"></div></div>"
  );


  $templateCache.put('src/partials/esParamBoolean.html',
    "<label class=\"control-label es-param-label inline\" uib-tooltip={{::esParamDef.toolTip}} tooltip-placement=top tooltip-trigger=mouseenter>{{::esParamDef.caption}}</label><input type=checkbox class=inline id={{::esParamDef.id}} class=\"form-control es-param-control\" ng-model-options=\"{getterSetter: true}\" ng-model=\"esParamVal[esParamDef.id].pValue\">"
  );


  $templateCache.put('src/partials/esParamDateRange.html',
    "<div class=\"col-md-6 col-lg-4 col-sm-12\"><label class=\"control-label es-param-label\" uib-tooltip={{::esParamDef.toolTip}} tooltip-placement=top tooltip-trigger=mouseenter>{{::esParamDef.caption}}</label><select class=\"form-control es-param-control\" kendo-drop-down-list uib-tooltip=\"Hello World\" uib-tooltip-trigger=mouseenter k-data-text-field=\"'title'\" k-auto-bind=true k-data-value-field=\"'dValue'\" k-data-source=\"::esGlobals.getesDateRangeOptions({dateRangeClass: esParamDef.controlType})\" k-value-primitive=true k-ng-model=esParamVal[esParamDef.id].paramValue.dRange></select></div><div class=\"col-md-12 col-sm-6\"><span class=\"col-md-4 col-sm-6 col-lg-4 nopadding-r\" ng-hide=\"esParamVal[esParamDef.id].paramValue.dRange > '1'\"><input class=\"form-control es-param-control margin-t\" kendo-date-picker k-ng-model=\"esParamVal[esParamDef.id].paramValue.fromD\"></span> <span class=\"col-md-4 col-sm-6 col-lg-4 date-margin nopadding-r\" ng-hide=\"esParamVal[esParamDef.id].paramValue.dRange != '0'\"><input class=\"form-control es-param-control margin-t\" kendo-date-picker k-ng-model=\"esParamVal[esParamDef.id].paramValue.toD\"></span></div>"
  );
  ...
  $templateCache.put('src/partials/esParams.html',
    "<uib-accordion><div uib-accordion-group class=panel-default is-open=esPanelOpen><uib-accordion-heading>{{::(esForceTitle ? esForceTitle : esParamsDef.title)}}&nbsp; <i class=\"pull-right glyphicon\" ng-class=\"{'glyphicon-triangle-bottom': esPanelOpen, 'glyphicon-triangle-right': !esPanelOpen}\"></i></uib-accordion-heading><form novalidate class=form name=esParamsPanelForm><div class=row><div class=\"form-group col-md-12 col-sm-12 col-xs-12 col-lg-5 smaller\" ng-class=\"{'has-error': esParamsPanelForm.{{::param.id}}.$invalid}\" ng-repeat=\"param in ::esParamsDef.simpleDefinitions() | orderBy:'aa'\"><es-param class=es-param es-type=\"param | esParamTypeMapper\" es-param-val=esParamsValues es-param-def=param></es-param></div><div ng-if=\"esParamsDef.hasAdvancedParams() && !esMore\" class=\"form-group col-md-3 more-cont\"><label class=\"control-label es-param-label more\">...</label><button type=button class=\"btn btn-info form-control es-param-control\" ng-click=esToggleMore()>{{ esMore ? ('ESUI.PARAMS_PANEL.Less' | translate) : ('ESUI.PARAMS_PANEL.More' | translate)}}</button></div></div><div class=row><div ng-if=esMore class=\"form-group col-md-3 col-sm-6 col-xs-12\" ng-class=\"{'has-error': esParamsPanelForm.{{::param.id}}.$invalid}\" ng-repeat=\"param in esParamsDef.advancedDefinitions() | orderBy:'aa'\"><es-param class=es-param es-type=\"param | esParamTypeMapper\" es-param-val=esParamsValues es-param-def=param></es-param></div><div ng-if=esMore class=\"form-group col-md-4 more-cont\"><label class=\"control-label es-param-label more\">...</label><button type=button class=\"btn btn-info form-control es-param-control\" ng-click=esToggleMore()>{{ esMore ? ('ESUI.PARAMS_PANEL.Less' | translate) : ('ESUI.PARAMS_PANEL.More' | translate)}}</button></div></div></form><div class=row><div ng-if=esShowRun class=\"col-md-12 col-sm-12 padding-15t\"><button type=button class=\"btn btn-primary\" ng-click=esRunClick()>{{::esRunTitle}}</button></div></div></div></uib-accordion>"
  );
