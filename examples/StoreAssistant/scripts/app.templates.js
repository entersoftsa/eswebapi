angular.module('esStoreAssistant').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/appmenu.html',
    "<nav class=\"navbar navbar-inverse navbar-fixed-top\"><div class=container-fluid><div class=navbar-header><button type=button class=\"navbar-toggle collapsed\" data-toggle=collapse data-target=#es-navbar aria-expanded=false><span class=sr-only>Toggle navigation</span> <span class=icon-bar></span> <span class=icon-bar></span> <span class=icon-bar></span></button> <a class=navbar-brand href=#><img alt=\"Entersoft SA\" src=https://www.entersoft.gr/esaddin/eslogo32.png></a></div><div class=\"collapse navbar-collapse\" id=es-navbar><ul class=\"nav navbar-nav\"><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{'MENU.SALES.TITLE' | translate}}<span class=caret></span></a><ul class=dropdown-menu><li><a ui-sref=sales_messages>{{'MENU.SALES.MESSAGES' | translate}}</a></li><li><a ui-sref=sales_issues>{{'MENU.SALES.ISSUES' | translate}}</a></li><li><a ui-sref=sales_stores>{{'MENU.SALES.STORES' | translate}}</a></li></ul></li><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{'MENU.SKU.TITLE' | translate}}<span class=caret></span></a><ul class=dropdown-menu><li><a ui-sref=skuAvailability>{{'MENU.SKU.AVAILABILITY' | translate}}</a></li><li><a ui-sref=skuPrice>{{'MENU.SKU.PRICE' | translate}}</a></li><li><a ui-sref=skuInfo>{{'MENU.SKU.INFORMATION' | translate}}</a></li></ul></li><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{'MENU.CUSTOMER.TITLE' | translate}}<span class=caret></span></a><ul class=dropdown-menu><li><a ui-sref=customer_search>{{'MENU.CUSTOMER.INFORMATION' | translate}}</a></li><li><a ui-sref=customer_new>{{'MENU.CUSTOMER.NEW' | translate}}</a></li></ul></li><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{'MENU.DOCS.TITLE' | translate}}<span class=caret></span></a><ul class=dropdown-menu><li><a ui-sref=docsOrders>{{'MENU.DOCS.ORDERS' | translate}}</a></li><li><a ui-sref=docsReturns>{{'MENU.DOCS.RETURNS' | translate}}</a></li></ul></li><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{'MENU.STORE.TITLE' | translate}}<span class=caret></span></a><ul class=dropdown-menu><li><a ui-sref=storeSales>{{'MENU.STORE.SALES' | translate}}</a></li><li><a ui-sref=storeScore>{{'MENU.STORE.SCORE' | translate}}</a></li></ul></li></ul><div class=\"navbar-text navbar-right\"><img aria-hidden=true ng-src={{theSession.UserLogoUrl}} style=\"display: inline; width: 32px; height: 32px\"></div><ul class=\"nav navbar-nav navbar-right\"><li class=dropdown><a class=dropdown-toggle data-toggle=dropdown role=button aria-haspopup=true aria-expanded=false>{{theSession.UserName}}<span class=caret></span></a><ul class=dropdown-menu><li><a ng-click=changePassword()>{{'MENU.CHGPWD' | translate}}</a></li><li><a ng-click=signOut()>{{'MENU.SIGNOUT' | translate}}</a></li></ul></li></ul></div></div></nav><div ui-view>"
  );


  $templateCache.put('views/customer_new.html',
    "<div class=\"container esContainer\"><es-params-panel es-params-def=vm.customerFields es-params-values=vm.customerValues es-panel-open=vm.status es-run-click=vm.addCustomer() es-run-title=Add es-show-run=true></es-params-panel></div>"
  );
  ...
  $templateCache.put('views/sales_Issues.html',
    "<h1>Hello World</h1>"
  );


  $templateCache.put('views/sales_Messages.html',
    "<div class=container><uib-accordion close-others=true><div ng-repeat=\"g in vm.groupMessages\" uib-accordion-group ng-class=g.class is-open=g.isOpen><uib-accordion-heading>{{g.title}} <span class=badge>{{g.messages.length}}</span> <i class=\"pull-right glyphicon\" ng-class=\"{'glyphicon-chevron-down': g.isOpen, 'glyphicon-chevron-right': !g.isOpen}\"></i></uib-accordion-heading><div class=qa-message-list id=wallmessages><div ng-repeat=\"msg in g.messages\" class=message-item id=msg.Code><div class=message-inner><div class=\"message-head clearfix\"><div class=\"avatar pull-left\"><img ng-src={{msg.personLogo}}></div><div class=user-detail><h5 class=handle>{{msg.Title}}</h5><div class=post-meta><div class=asker-meta><span class=qa-message-what></span> <span class=qa-message-when><span class=qa-message-when-data>{{msg.ReleaseDate}}</span> </span><span class=qa-message-who><span class=qa-message-who-pad>by </span><span class=qa-message-who-data>{{msg.Owner}}</span></span></div></div></div></div><div class=qa-message-content ng-bind-html=msg.Body></div></div></div></div></div></uib-accordion></div>"
  );


  $templateCache.put('views/sales_Stores.html',
    "<h1>Hello World</h1>"
  );


  $templateCache.put('views/sku_availability.html',
    "<div class=container><uib-accordion close-others=true><div uib-accordion-group class=panel-primary heading=\"1. Search ...\" is-open=vm.status.searchisOpen><form name=search_sku class=form><div class=row><div class=\"form-group col-xs-12\"><input id=searchctrl class=form-control placeholder=\"Barcode, Code, Multiple Codes\" ng-required=true ng-model-options=\"{getterSetter: true}\" ng-model=vm.searchValue.pValue></div></div><button class=\"btn btn-primary\" type=button ng-disabled=\"search_sku.$invalid || vm.status.isrunning\" ng-click=vm.searchSKU()>{{'ESUI.GENERIC.OK' | translate}}</button></form></div><div uib-accordion-group class=panel-danger heading=\"2. Local Store\" is-open=vm.status.storeisOpen is-disabled=!vm.myStoreRow><table class=\"table table-hover table-condensed\"><thead><tr><th>{{'TABLE.FIELD' | translate}}</th><th>{{'TABLE.VALUE' | translate}}</th></tr></thead><tbody><tr><td>Scan Code</td><td>{{vm.searchValue.strVal()}}</td></tr><tr ng-repeat=\"col in vm.myStoreInfo.columns\"><td>{{col.title}}</td><td>{{vm.myStoreRow[col.field]}}</td></tr></tbody></table></div><div uib-accordion-group class=panel-success heading=\"3. Other Stores\" is-open=vm.status.otherisOpen is-disabled=!vm.myStoreRow><es-grid es-group-id=vm.otherDef.GroupID es-filter-id=vm.otherDef.FilterID es-execute-params=vm.otherDef.Params es-pq-options=vm.otherDef.PQOptions es-data-source=vm.otherDS></div></uib-accordion></div>"
  );
