...
(function() {
    ...
    esWEBUI
        ...
        .filter('esParamTypeMapper', ['esUIHelper', function(esUIHelper) {
            var f = function(pParam) {
                if (!pParam || !(pParam instanceof esUIHelper.ESParamInfo)) {
                    return "";
                }
    ...
    esWEBUI
            ...
            function($scope, $log, $window, esMessaging, esWebApiService, esWebUIHelper, esGlobals, $translate, $uibModal, $q, esCache, $timeout, clipboard) {
                ...
                function processesDef(esDef, lang) {
                    ...
                    if (esDef && esDef.Description) {
                        esDef.Description = esTranslate(esDef.Description, lang);
                    }
                    ...
                    $scope.globalExecutePQ = function(a, b) {
                        ...
                        _.forEach($scope.esPqDef, function(section) {
                            _.forEach(section, function(dbItem) {
                                _.forEach(globalParamsDefs.paramsValues, function(globalParam) {
                                    var x = dbItem.Params[globalParam.paramCode];
                                    if (x) {
                    ...
                    if ($scope.esPqDef.GlobalParamsPanel) {
                        ...
                        _.forEach(fin, function(i) {
                            _.forEach(i, function(dbItem) {
                                if (dbItem.UIOptions && dbItem.UIOptions.autoBind) {
                                    dbItem.UIOptions.autoBind = false;
                                }

                                if (dbItem.PQOptions && dbItem.PQOptions.AutoExecute) {
                        ...
                        fin.GlobalParamsPanel = {
                            AutoExecute: $scope.esPqDef.GlobalParamsPanel.AutoExecute,
                            esPanelOpen: {
                                status: $scope.esPqDef.GlobalParamsPanel.esPanelOpen && $scope.esPqDef.GlobalParamsPanel.esPanelOpen.status
                            },
        ...
        .directive('esComboPq', ['$log', 'esWebApi', 'esMessaging', 'esUIHelper', 'esGlobals',
        ...
        .directive('esGanttPq', ['$log', '$window', 'esWebApi', 'esMessaging', 'esUIHelper', 'esGlobals', '$translate',
        ...
        .directive('esTimelinePq', ['$log', '$window', '$timeout', 'esWebApi', 'esMessaging', 'esUIHelper', 'esGlobals', '$translate',
            function($log, $window, $timeout, esWebApiService, esMessaging, esWebUIHelper, esGlobals, $translate) {
                return {
                    restrict: 'AE',
                    scope: {
                        esPqDef: "=",
                    ...
                    link: function($scope, iElement, iAttrs) {
                                    ...
                                    read: function(options) {
                                        ...
                                        esWebApiService.fetchPublicQuery($scope.esPqDef.GroupID, $scope.esPqDef.FilterID, pqOptions, executeParams)
                                            .then(function(pq) {
                                                ...
                                                if ($scope.esPqDef.UIOptions && angular.isString($scope.esPqDef.UIOptions.TimelineEntityInfoPQ)) {
                                                    var parts = $scope.esPqDef.UIOptions.TimelineEntityInfoPQ.split(",");
                                                    parts[0] = parts[0].trim();
                                                    if (parts[0]) {
                                                        sG = parts[0];
                                                    }
                                                ...
                                                esWebApiService.fetchPublicQuery(sG, fG, pqOptions, executeParams)
                                                    .then(function(master) {
                                                        if (master && master.data && master.data.Rows && master.data.Rows.length) {
                                                            $scope.entityDS = master.data.Rows[0];
                                                        } else {
                                                            $scope.entityDS = {};
                                                        }
                                ...
                                schema: {
                                    model: {
                                        fields: {
                                            ...
                                            title: { from: "Title", type: "string" },
                                            titleAmount: { from: "TitleAmount", type: "number" },
                                            priority: { from: "Priority", type: "number" },
                                            latitude: { from: "Latitude", type: "number" },
                                            longitude: { from: "Longitude", type: "number" },
                        ...
                        var opts = {
                            ...
                            eventTemplate: '<div class="timeline-event-container" data-priority="#= data.priority #">' +
                            ...
                            dataBound: function(e) {
                                $('.timeline-event-container').each(function() {
                                    var $this = $(this);

                                    $this.closest('.k-timeline-event').addClass('k-timeline-event-priority-' + ($this.data('priority') || 0));

                                    if ($.trim($this.find('.k-card-body').html()) === '')
                                        $this.addClass('timeline-event-container-nobody');
                                });
                            },
                            expand: function(e) {
                                ...
                                if ((e.dataItem.latitude != null) && (e.dataItem.longitude != null) && ($mapContainer.data('kendoMap') == null)) {
                                    $mapContainer.kendoMap({
                                        ...
                                        center: [e.dataItem.latitude, e.dataItem.longitude],
                                        zoom: 15,
                                        markers: [{
                                            location: [e.dataItem.latitude, e.dataItem.longitude]
                                        }]
                                    });
                                }
                            }
            ...
            function($log, $window, $uibModal, $timeout, esWebApiService, esMessaging, esWebUIHelper, esGlobals) {
                return {
                    ...
                    link: function($scope, iElement, iAttrs) {
                        ...
                        $scope.cardDataSource = esWebUIHelper.getPQDataSource($scope.esPqDef, dsOptions);
                ...
                return {
                    ...
                    link: function($scope, iElement, iAttrs) {
                        ...
                        if ($scope.esGroupId instanceof esGlobals.ESPublicQueryDef && !iAttrs.esParamsValues) {
                            ...
                            $scope.esGroupId.ParametersDefinition = function() {
                                return $scope.esParamsDef;
                            }
                        }
    ...
    esWEBUI.factory('esUIHelper', ['$state', '$translate', '$log', '$timeout', 'esMessaging', 'esWebApi', 'esGlobals',
        function($state, $translate, $log, $timeout, esMessaging, esWebApiService, esGlobals) {
            ...
            function ESParamInfo(p) {
                ...
                this.parameterType = (p && p.parameterType) ? p.parameterType : "";
                ...
                this.invSelectedMasterTable = p ? p.invSelectedMasterTable : undefined;
                this.invSelectedMasterField = p ? p.invSelectedMasterField : undefined;
                ...
                this.defaultValues = p ? getEsParamVal(this, p.defaultValues) : undefined;
            ...
            ESParamsDefinitions.prototype.createDefinitions = function(title, params) {
                this.title = title;
